...
- 智能推荐生成方式（标准CRUD自动使用生成器，节省60% Token）
- 自动代码审查与修复
- 已有代码检测与跳过机制
- 完整的回滚和错误处理
...
| 4 | 文档初始化 | 低 | TODO和设计文档 |
| 5 | 数据库设计 | 中 | 表结构和SQL |
| 5.5 | 代码生成决策 | 极低 | 智能推荐生成方式 |
| 6 | 后端开发 | 低-中 | Controller/Service/Mapper |
| 7 | 前端开发 | 中 | Vue3组件和API |
| 8 | 测试验证 | 中 | 单元测试和集成测试 |
| 9 | 质量检查 | 低 | 代码规范和安全扫描 |
| 9.5 | 性能分析 | 极低 | SQL优化和内存检测 |
| 10 | 文档更新 | 极低 | 更新README和API文档 |
| 11 | Git提交 | 极低 | 代码提交和推送 |
...
| 方式 | Token消耗 | 适用场景 | 优势 |
|------|----------|---------|------|
| A（全生成器） | 5,900 (节省53%) | 标准CRUD | 快速、规范、Token少 |
| C3（后端生成器+前端AI） | 8,600 (节省31%) | 标准后端+定制UI | 平衡 |
| C4（后端AI+前端生成器） | 9,800 (节省22%) | 复杂逻辑+标准UI | 灵活 |
| B（全AI） | 12,500 (基准) | 复杂业务 | 高度定制 |
...
```bash
# 1. 检测数据库SQL脚本
find docs/sql -name "*.sql" -type f 2>/dev/null
find scripts/sql -name "*.sql" -type f 2>/dev/null

# 2. 检测后端代码
find ruoyi-modules -name "*Controller.java" -type f 2>/dev/null

# 3. 检测前端代码
find plus-ui/src/views -name "index.vue" -type f 2>/dev/null
```
...
- **设计产出**：
	...
	4. **数据流转模式**
		...
		- 规划缓存策略（如需要）
	5. **已有代码映射清单**（如步骤1检测到已有代码）
		- 如果步骤1检测到已有代码，在设计文档中记录：
		```markdown
		### 已有代码清单
		
		检测时间：2026-02-02 10:30
		
		代码位置：
		- SQL脚本: docs/sql/order_management.sql
		- 后端Controller: ruoyi-business-order/controller/OrderController.java
		- 后端Service: ruoyi-business-order/service/IOrderService.java
		- 后端Mapper: ruoyi-business-order/mapper/OrderMapper.java
		- 前端页面: plus-ui/src/views/business/order/index.vue
		- 前端API: plus-ui/src/api/business/order.ts
		
		代码状态评估：
		- 与需求匹配度: [高/中/低]
		- 代码质量: [优秀/良好/需优化/需重构]
		- 数据库表结构完整性: [完整/部分完整/缺失]
		
		推荐操作：
		- 步骤5（数据库设计）: ✅ 跳过，直接使用已有SQL
		- 步骤6（后端开发）: ✅ 跳过生成，仅代码审查
		- 步骤7（前端开发）: ✅ 跳过生成，仅代码审查
		- 步骤8（测试验证）: ⚠️ 必须执行
...
#### 检测范围
1. **已有代码检测**：检测用户是否已有完整代码
2. **代码生成器检测**：检测若依代码生成器是否可用
...
# 1. 检测后端代码
find ruoyi-modules -name "*Controller.java" -type f 2>/dev/null
find ruoyi-modules -name "*Service.java" -type f 2>/dev/null
find ruoyi-modules -name "*Mapper.java" -type f 2>/dev/null

# 2. 检测前端代码
find plus-ui/src/views -name "index.vue" -type f 2>/dev/null
find plus-ui/src/api -name "*.ts" -type f 2>/dev/null
...
```bash
# 1. 检测生成器模块是否存在
ls ruoyi-modules/ruoyi-generator 2>/dev/null

# 2. 检测生成器依赖是否配置
grep -r "ruoyi-generator" pom.xml 2>/dev/null
```

#### 检测结果处理

- **情况1：检测到完整代码**（后端+前端+SQL都存在）
	```
	🔍 检测到已有完整代码
	代码清单：
	- 后端Controller: ruoyi-modules/ruoyi-business-order/controller/OrderController.java
	- 后端Service: ruoyi-modules/ruoyi-business-order/service/IOrderService.java
	- 后端Mapper: ruoyi-modules/ruoyi-business-order/mapper/OrderMapper.java
	- 前端页面: plus-ui/src/views/business/order/index.vue
	- 前端API: plus-ui/src/api/business/order.ts
	- SQL脚本: docs/sql/order_management.sql
	...
	💡 建议操作：
	1. 【直接使用】⭐ 推荐
	   - 跳过步骤5-7，直接进入步骤8（测试验证）
	   - Token节省：~5,000 tokens
	   - 时间节省：~60分钟
	
	2. 【审查优化】
	   - AI分析现有代码质量
	   - 提供优化建议（性能、安全、规范）
	   - Token消耗：~1,500 tokens
...
- **情况2：检测到部分代码**
	...
	🔍 检测到部分代码
	已有代码：
	✅ 后端代码: 完整（Controller/Service/Mapper都存在）
	✅ SQL脚本: docs/sql/order_management.sql
	❌ 前端代码: 不存在
...
- **执行分支**：
	- **分支A1：检测到完整后端代码（用户选择"直接使用"）**
		- ✅ 跳过代码生成
		- ✅ 执行代码审查（@code-reviewer）
		- ✅ 记录代码位置到步骤记录
		- ⏭️ 直接进入步骤7
