...
```
检查项：
1. 检查 ruoyi-modules/ 下已存在的业务模块
2. 确认设计方案中的模块名称

处理：
- 如果项目中尚无业务模块：
  → 建议使用 ruoyi-business-[业务名] 格式（如 ruoyi-business-order）
  → 提供选项：1) 使用建议格式  2) 使用自定义名称
  → 根据用户选择确定最终模块名
...
```
多模块创建场景：
- 如需创建多个新模块，必须逐个创建，不可并行处理
- 每个模块都执行完整的前置检查和创建流程
- 完成一个模块的全部步骤后，再开始下一个模块

单模块检查项：
1. 模块目录是否存在：ruoyi-modules/ruoyi-[模块名]/
2. pom.xml 是否存在：ruoyi-modules/ruoyi-[模块名]/pom.xml
